 ...
 * In "markdown" mode it uses GitHub alert callouts; in "plain" mode it
 * uses a simple "Severity: message" prefix.
 */
export function formatBody(
    ...
    const escaped = Bun.escapeHTML(message);
    if (format === "markdown") {
...
export function toComments(
    ...
    return lints.map((l) => ({
        path: l.filePath,
        line: l.line,
        body: formatBody(l.severity, l.message, format),
    }));
}
